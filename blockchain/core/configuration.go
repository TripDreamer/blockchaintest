...
func getDirectoryWithBaseDir(dir string) string {
	...
	if dir == HOME_DIRECTORY_CONFIG {
		...
		usr, err := user.Current() //return the current user
		logOnError(err)
		dir = usr.HomeDir // HomeDir is the path to the user's home directory (if they have one)
	}

	return path.Join(dir, BLOCKCHAIN_DIRECTORY) //Join BLOCKCHAIN_DIRECTORY after dir  :  dir/BLOCKCHAIN_DIRECTORY
}
...
func OpenConfiguration(dir string) (*Keypair, error) {
	...
	err := os.MkdirAll(dir, 0777) //set quanxian
	logOnError(err)

	//Search for keys file inside directory
	f, err := os.OpenFile(path.Join(dir, BLOCKHAIN_KEYS_FILENAME), os.O_RDWR|os.O_CREATE, 0660) //open key file,if not exist,then create it
	defer f.Close()
